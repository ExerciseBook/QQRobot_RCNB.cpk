...
implementation
uses
    SysUtils, Classes, RCNB, RCNB_Helper;
...
Begin
	...
	if RCNB_Helper.RCNB_Helper(msg,back) then begin
		CQ_i_sendPrivateMSG(fromQQ,back);
	end;
...
Begin
	...
	if RCNB_Helper.RCNB_Helper(msg,back) then begin
		CQ_i_sendGroupMsg(fromGroup,widestring(CQCode_Group_At(fromQQ))+ ' ' + back);
	end;
...
Begin
	...
	if RCNB_Helper.RCNB_Helper(msg,back) then begin
		CQ_i_sendDiscussMsg(fromDiscuss,widestring(CQCode_Group_At(fromQQ))+ ' ' + back);
	end;
